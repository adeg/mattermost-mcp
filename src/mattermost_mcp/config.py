...
class LlmConfig(BaseSettings):
    ...
    max_tokens: int = Field(default=1000, alias="ANTHROPIC_MAX_TOKENS")
...
class MonitoringConfig(BaseSettings):
    ...
    schedule: str = Field(default="*/5 * * * *", alias="MONITORING_SCHEDULE")
    channels: list[str] = Field(default_factory=list, alias="MONITORING_CHANNELS")
    topics: list[str] = Field(default_factory=list, alias="MONITORING_TOPICS")
    message_limit: int = Field(default=50, alias="MONITORING_MESSAGE_LIMIT")
    state_path: str = Field(default="./monitor-state.json", alias="MONITORING_STATE_PATH")
    process_existing_on_first_run: bool = Field(default=False, alias="MONITORING_PROCESS_EXISTING")
    ...
    def from_env(cls) -> "MonitoringConfig":
        ...
        channels_str = os.getenv("MONITORING_CHANNELS", "")
        topics_str = os.getenv("MONITORING_TOPICS", "")
